...
('reviewer@compliancefirst.com', 'Lisa Reviewer', 'reviewer', 'Quality Assurance', 40, 10),
...
WHERE t.name = 'Tax Compliance Team' AND u.email IN ('employee1@compliancefirst.com', 'employee2@compliancefirst.com', 'reviewer@compliancefirst.com');
...
('Audit Support', 'Preparation of documents for audit', 20, 'reviewer', 15);
...
((SELECT id FROM public.tasks WHERE title = 'Audit Documents - GHI Ltd'), (SELECT id FROM public.users WHERE email = 'reviewer@compliancefirst.com'), (SELECT id FROM public.users WHERE email = 'manager@compliancefirst.com')),
...
INSERT INTO public.subtasks (task_id, title, is_done, sort_order, completed_by) VALUES
((SELECT id FROM public.tasks WHERE title = 'Audit Documents - GHI Ltd'), 'Gather financial statements', true, 1, (SELECT id FROM public.users WHERE email = 'reviewer@compliancefirst.com')),
((SELECT id FROM public.tasks WHERE title = 'Audit Documents - GHI Ltd'), 'Prepare reconciliation sheets', true, 2, (SELECT id FROM public.users WHERE email = 'reviewer@compliancefirst.com')),
...
((SELECT id FROM public.tasks WHERE title = 'Audit Documents - GHI Ltd'), (SELECT id FROM public.users WHERE email = 'reviewer@compliancefirst.com'), 'All financial statements are ready. Just need final approval on supporting docs.'),
...
((SELECT id FROM public.users WHERE email = 'reviewer@compliancefirst.com'), (SELECT id FROM public.tasks WHERE title = 'Audit Documents - GHI Ltd'), 'task_review', 'Task Ready for Review', 'Please review: Audit Documents - GHI Ltd', false);